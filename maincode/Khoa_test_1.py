...
from detect_edges_image import edgeDetection
import os.path
import csv
import sys
import threading

INPUT_VIDEO = 'finalNCKHclip.mp4'
...
def check_label(img1):
    ...
    return get_template (sample_gray)
    # key = cv2.waitKey(0) & 0xFF
    # if key == ord("q"):
    #     cv2.destroyAllWindows()

def check_bottle(frame):
    global ROI, objects, text_1, initROI, count, check_shape
    for (objectID, centroid) in objects.items():
        ...
        if to is None:
            ...
            roi_label = roi[int(ROI[1][1]):int(ROI[1][1]+ROI[1][3]), int(ROI[1][0]):int(ROI[1][0]+ROI[1][2])]
            cv2.imwrite('crop.jpg', roi_label)
            # check_label(roi)
            thread_check_location = ThreadWithReturnValue(target=check_label,args=(roi,))
            thread_check_location.start()
            thread_check_shape = ThreadWithReturnValue(target=edgeDetection.checkShape, args=(check_shape, roi_label))
            thread_check_shape.start()
            number_angles = thread_check_shape.join()
            res_label_location = thread_check_location.join()
            print(res_label_location)
            print(number_angles)
            if (res_label_location == 4) and (number_angles == SHAPE_OF_LABEL):
                text_1 = "ID {}: GOOD Label".format(objectID)
                count+= 1
            elif (res_label_location == 4) and (number_angles != SHAPE_OF_LABEL):
                text_1 = "ID {}: Label is Folded".format(objectID)
            else:
                text_1 = "ID {}: Label is Wrong ".format(objectID)
...
subtractor = cv2.bgsegm.createBackgroundSubtractorMOG(history=800)
...
while True:
  ...
  if ret == True:
    ...
    copy_h, copy_w = frame_copy.shape[:2]
    #gray = cv2.cvtColor(mask_hsv, cv2.COLOR_HSV2GRAY)
    # mask = cv2.GaussianBlur(mask_hsv, (5,5), 0)
    # if firstframe is None:
    #     firstframe = mask_hsv
    #     continue
    # frameDelta = cv2.absdiff(firstframe, mask_hsv)
    mask = cv2.threshold(mask_hsv, 25, 255, cv2.THRESH_BINARY)[1]
    #mask = frameDelta
    #mask = subtractor.apply(mask_hsv,1)
    ...
    for c in cnts:
        ...
        if (w >= 100) and (h>=150):
            ...
            if initROI == False and cx > 250:
                ROI = []
                clone = cv2.resize(frame,(int(frame.shape[1]*0.5),int(frame.shape[0]*0.5)))
                ROI.append(tuple([2*x for x in cv2.selectROI("Select the zone for checking", clone, False)]))
                qlog = open( 'ROI.csv', 'w' )
                qlog.write('%d\t%d\t%d\t%d\n' %(ROI[0][0], ROI[0][1], ROI[0][2], ROI[0][3]))
                ROI_product = frame[int(ROI[0][1]):int(ROI[0][1]+ROI[0][3]), int(ROI[0][0]):int(ROI[0][0]+ROI[0][2])]
                clone = cv2.resize(ROI_product,(int(ROI_product.shape[1]*0.5),int(ROI_product.shape[0]*0.5)))
                ROI.append(tuple([2*x for x in cv2.selectROI("Select the zone for checking", clone, False)]))
